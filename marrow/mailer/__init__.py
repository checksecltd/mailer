...
class Mailer(object):
    ...
    def __repr__(self):
        return "Mailer(manager=%s, transport=%s)" % (
            self.Manager.__name__,
            self.Transport.__name__,
        )
    ...
    def __init__(self, config, prefix=None):
        ...
        if not Manager:
            raise LookupError(
                "Unable to determine manager from specification: %r" % (config.manager,)
            )
        ...
        self.Transport = Transport = self._load(
            transport_config.use, "marrow.mailer.transport"
        )
...
class Delivery(Mailer):
    def __init__(self, *args, **kw):
        warnings.warn(
            "Use of the Delivery class is deprecated; use Mailer instead.",
            DeprecationWarning,
        )
