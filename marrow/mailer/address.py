...
class Address(object):
    ...
    def __init__(self, name_or_email, email=None, encoding="utf-8"):
        ...
        if err:
            raise ValueError(
                '"{0}" is not a valid e-mail address: {1}'.format(email, err)
            )
    ...
    def __eq__(self, other):
        ...
        raise NotImplementedError(
            "Can not compare Address instance against {0} instance".format(type(other))
        )
    ...
    def __repr__(self):
        return 'Address("{0}")'.format(
            unicode(self).encode("ascii", "backslashreplace").decode("ascii")
        )
...
class AddressList(list):
    def __init__(self, addresses=None, encoding="utf-8"):
        ...
        if not isinstance(addresses, list):
            raise ValueError(
                "Invalid value for AddressList: {0}".format(repr(addresses))
            )
    ...
    def __setitem__(self, k, value):
        if isinstance(k, slice):
            value = [
                Address(val) if not isinstance(val, Address) else val for val in value
            ]
    ...
    def extend(self, sequence):
        values = [
            Address(val) if not isinstance(val, Address) else val for val in sequence
        ]
