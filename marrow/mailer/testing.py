...
TestMessage = namedtuple(
    "TestMessage", ("sender", "recipients", "time", "message", "raw")
)
...
class DebuggingSMTPServer(SMTPServer, Thread):
    ...
    def main(cls):
        ...
        print(
            "Debugging SMTP server is running on ",
            server.address[0],
            ":",
            server.address[1],
            sep="",
    ...
    def process_message(self, peer, sender, recipients, data):
        ...
        message = TestMessage(
            sender, recipients, datetime.utcnow(), Parser().parsestr(data), data
        )
