...
class Message(object):
    ...
    def envelope(self):
        ...
        if not self.sender and not self.author:
            raise ValueError(
                "Unable to determine message sender; no author or sender defined."
            )
    ...
    def attach(
        ...
        if inline:
            if sys.version_info < (3, 0):
                ...
                part.add_header(
                    "Content-ID".encode("utf-8"), "<%s>".encode("utf-8") % filename
                )
            else:
    ...
    def send(self):
        if not self.mailer:
            raise NotImplementedError(
                "Message instance is not bound to "
