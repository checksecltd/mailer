...
class KeywordProcessor(object):
    ...
    def __init__(
        ...
        self.pattern = "".join(
            (
                (
                    r"[\s%s]*" % ("".join(separators),)
                ),  # Trap possible leading space or separators.
                "(",
                (
                    "[%s]%s"
                    % (
                        "".join([i for i in list(groups) if i is not None]),
                        "?" if None in groups else "",
                    )
                )
                if groups
                else "",  # Pass groups=('+','-') to handle optional leading + or -.
                "".join([(r"%s[^%s]+%s|" % (i, i, i)) for i in quotes])
                if quotes
                else "",  # Match any amount of text (that isn't a quote) inside quotes.
                (
                    "[^%s]+" % ("".join(separators),)
    ...
    def join(self, values):
        ...
        if self.group is dict:
            if not isinstance(values, dict):
                raise ValueError(
                    "Dictionary grouped values must be passed as a dictionary."
                )  # pragma: no cover

            return self.separators[0].join(
                [
                    (prefix + sanatize(keyword))
                    for prefix, keywords in values
                    for keyword in values[prefix]
                ]
            )

        if not isinstance(values, (list, tuple, set)):
            raise ValueError(
                "Ungrouped values must be passed as a list, tuple, or set."
            )

        return self.separators[0].join([sanatize(keyword) for keyword in values])


tags = KeywordProcessor(
    " \t,", normalize=lambda s: s.lower().strip('"'), sort=True, result=set
)
tags.__doc__ = (
    "A lowercase-normalized ungrouped tagset processor, returning only unique tags."
)
