...
class TestBasicMessage(TestCase):
    ...
    def test_non_ascii_attachment_names(self):
        ...
        if sys.version_info < (3, 0):
            assert "filename*=\"utf-8''%E2%98%83.txt\"" in unicode(
                message
            )  # ☃ is encoded in ASCII as \xe2\x98\x83, which is URL encoded as %E2%98%83
        else:
            assert "filename*=utf-8''%E2%98%83.txt" in unicode(
                message
            )  # ☃ is encoded in ASCII as \xe2\x98\x83, which is URL encoded as %E2%98%83
    ...
    def test_language_specification_and_charset_for_attachment_name(self):
        ...
        if sys.version_info < (3, 0):
            assert "filename*=\"utf-8'en-us'%E2%98%83.txt\"" in unicode(
                message
            )  # ☃ is encoded in ASCII as \xe2\x98\x83, which is URL encoded as %E2%98%83
        else:
            assert "filename*=utf-8'en-us'%E2%98%83.txt" in unicode(
    ...
    def test_that_add_header_and_collapse_header_are_inverses_ascii_filename(self):
        ...
        message.rich = "Farewell cruel world."
        message.attach(
            "wat.txt", b"not a unicode snowman"
        )  # calls add_header() under the covers
        attachment = message.attachments[0]
        filename = (
            attachment.get_filename()
        )  # calls email.utils.collapse_rfc2231_value() under the covers
    ...
    def test_that_add_header_and_collapse_header_are_inverses_non_ascii_filename(self):
        ...
        message.attach("☃.txt", b"unicode snowman", filename_language="en-us")
        attachment = message.attachments[0]
        filename = attachment.get_param(
            "filename", object(), "content-disposition"
        )  # get_filename() calls this under the covers
        assert isinstance(
            filename, tuple
        )  # Since attachment encoded according to RFC2231, should be represented as a tuple
        filename = (
            attachment.get_filename()
        )  # Calls email.utils.collapse_rfc2231_value() under the covers, currently fails
        if sys.version_info < (3, 0):
            assert isinstance(
                filename, basestring
            )  # Successfully converts tuple to a string
        else:
    ...
    def test_recipients_collection(self):
        ...
        assert message.recipients.addresses == [
