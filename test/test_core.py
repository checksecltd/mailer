...
class TestLookup(TestCase):
    ...
    def test_load_dotcolon(self):
        assert (
            Mailer._load("marrow.mailer.manager.immediate:ImmediateManager", None)
            == ImmediateManager
        )
...
class TestInitialization(TestCase):
    def test_deprecation(self):
        with warnings.catch_warnings(record=True) as w:
            ...
            ), "Category of warning is not DeprecationWarning."
            assert "deprecated" in str(
                w[-1].message
            ), "Warning does not include 'deprecated'."
            assert "Mailer" in str(
                w[-1].message
            ), "Warning does not include correct class name."
            assert "Delivery" in str(
                w[-1].message
            ), "Warning does not include old class name."
    ...
    def test_prefix(self):
        config = {"mail.manager.use": "immediate", "mail.transport.use": "mock"}
    ...
    def test_manager_entrypoint_failure(self):
        config = {"manager.use": "immediate2", "transport.use": "mock"}
    ...
    def test_transport_entrypoint_failure(self):
        config = {"manager.use": "immediate", "transport.use": "mock2"}
...
class TestMethods(TestCase):
    ...
    def test_new(self):
        ...
        message = interface.new(
            "alternate@example.com", "recipient@example.com", "Test."
        )
